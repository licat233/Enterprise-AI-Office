...
对于高频密集结构产品（LED 灯珠、ESL 电子价签、货架照明），AI 容易生成"摩尔纹样伪影"。详细防伪影策略、正面/负面提示词模板、推荐工作流见：

→ `references/armor-anti-moire-guide.md`

核心原则：**降低 AI 需要猜的高频细节密度，比堆负面提示词有效得多。** 用 `smooth diffused luminous surface` 替代 `many LED beads`，产品放大、背景简化、避免过度锐化。

### 批量后处理脚本

脚本位于 `scripts/ai_texture_cleaner/`，可批量清理 AI 图片伪影。依赖：`pip install opencv-python pillow numpy`。三种强度：`light`（ESL/边缘）、`standard`（产品主体）、`strong`（LED 发光面/背景）。详见 `scripts/ai_texture_cleaner/README.md`。
