...
require_text config/validated-stack.yaml 'reference_runtime_last_confirmed_on: 2026-09-15' 'Validated stack records current reference runtime confirmation'
...
require_text README.md 'qwen3-embedding:0.6b' 'English README records current Qwen3 embedding baseline'
require_text README.md 'Qwen3-Reranker-0.6B' 'English README records current Qwen3 reranker baseline'
require_text README.md 'qwen3-vl:2b' 'English README records current Qwen3 vision baseline'
require_text README.md 'Qwen3-ASR' 'English README records current Qwen3 ASR baseline'
require_text README.md '9router/default' 'English README records WeKnora 9router KnowledgeQA path'
require_no_text README.md 'qwen2.5vl:3b<br/>Vision parsing' 'English README does not present legacy VLM as current'
require_no_text README.md 'bge-m3:latest' 'English README does not present legacy embedding as current'
require_no_text README.zh-CN.md 'qwen2.5vl:3b<br/>视觉解析' 'Chinese README does not present legacy VLM as current'
require_no_text README.zh-CN.md 'bge-m3:latest' 'Chinese README does not present legacy embedding as current'
require_text DEPLOY.md 'current local baseline, selected as part of the lightweight Qwen infrastructure standardization' 'Golden Path uses current Qwen3 embedding baseline'
require_text state/REAL-DEPLOYMENT-STATUS.md 'KnowledgeQA/Chat enabled: 9router / default' 'Reference status records current WeKnora KnowledgeQA path'
require_text state/REAL-DEPLOYMENT-STATUS.md 'Rerank           enabled: Qwen3-Reranker-0.6B' 'Reference status records current Qwen3 reranker'
require_text config/validated-stack.yaml 'container_image: wechatopenai/weknora-app:v0.8.0' 'Validated stack matches WeKnora runtime image identity'
require_text config/company.example.yaml 'system_aes_key: null' 'Company schema declares WeKnora SYSTEM_AES_KEY continuity ref'
require_text config/company.private.example.yaml 'native_binding: SYSTEM_AES_KEY' 'Private example binds WeKnora SYSTEM_AES_KEY'
require_text infrastructure/weknora/docker-compose.eaio.override.yml 'name: weknora' 'WeKnora adapter freezes Compose project identity'
require_text infrastructure/open-webui/docker-compose.yml 'name: eaio-openwebui' 'Open WebUI adapter freezes Compose project identity'
require_text DEPLOY.md '${RUNTIME_ROOT}/runtime/WeKnora' 'Golden Path uses canonical WeKnora runtime checkout'
require_no_text DEPLOY.md '${RUNTIME_ROOT}/upstream/WeKnora' 'Golden Path does not maintain a second active WeKnora checkout'
require_text DEPLOY.md 'SYSTEM_AES_KEY' 'Golden Path preserves WeKnora encryption continuity'
require_text scripts/core-lifecycle-smoke.sh 'WeKnora app recreate without frontend restart' 'Lifecycle smoke covers backend recreate'
require_text scripts/core-lifecycle-smoke.sh 'Recreate preserved all persistent-volume identities' 'Lifecycle smoke protects volume identity'
...
require_text config/validated-stack.yaml 'reference_tag: v2026.9.11' 'Validated stack records accepted Hermes reference tag'
require_text config/validated-stack.yaml 'provenance: scripts/install.sh_at_candidate_commit' 'Validated stack records candidate-scoped Hermes source-path provenance'
...
require_text scripts/backup.sh 'SYSTEM_AES_KEY="$(env_value SYSTEM_AES_KEY "$WEKNORA_ENV_FILE")"' 'Backup requires WeKnora encryption continuity key'
...
require_text infrastructure/hermes/PROVISIONING.md '### 3.1 Hermes behavior provenance — candidate-scoped' 'Hermes provisioning records candidate-scoped behavior provenance'
require_text config/validated-stack.yaml 'version_policy: rolling-validated' 'Hermes uses rolling-validated version policy'
require_text config/validated-stack.yaml 'permanent_version_pin: false' 'Hermes is not permanently version pinned'
require_text config/validated-stack.yaml 'resolve_candidate_once_per_change: true' 'Hermes freezes one candidate per change transaction'
require_text DEPLOY.md 'HERMES_CANDIDATE_COMMIT' 'Golden Path resolves a transaction-scoped Hermes candidate commit'
...
require_text docs/ACCEPTANCE-TESTS.md 'Hermes source checkout commit matches the transaction-scoped candidate commit' 'Core acceptance requires exact Hermes candidate identity'
...
require_text infrastructure/hermes/default.config.example.yaml 'Derived schema contract: config/validated-stack.yaml -> Hermes Agent rolling-validated candidate.' 'Hermes default example identifies rolling schema contract'
require_text infrastructure/hermes/general.config.example.yaml 'Derived schema contract: config/validated-stack.yaml -> Hermes Agent rolling-validated candidate.' 'Hermes general example identifies rolling schema contract'
require_no_text config/eao-manifest.yaml 'weknora: v0.8.0' 'Manifest does not duplicate WeKnora version'
require_no_text config/eao-manifest.yaml 'hermes_agent:' 'Manifest does not duplicate Hermes version/policy authority'
...
require_text docs/V2-PHASE-STATUS.md 'BLUEPRINT PHASE: RELEASE READY' 'v2 status matches current Release Ready phase'
...
require_text docs/V2-PHASE-STATUS.md 'ARMOR REFERENCE DEPLOYMENT: ACTIVE / DEPLOYED / HANDOFF-READY' 'v2 status records active handoff-ready ARMOR deployment'
...
require_text infrastructure/hermes/PROVISIONING.md 'PROVIDER_REGISTRY' 'Hermes provisioning resolves native binding from candidate provider registry'
...
require_text docs/ACCEPTANCE-TESTS.md 'selected Hermes provider auth path matches the transaction-scoped candidate provider registry/catalog' 'Core acceptance validates Hermes candidate provider auth binding'
...
require_text config/capabilities.yaml 'status: blocked_disabled' 'EAO registry records disabled Knowledge Mutation binding'
require_text config/capabilities.yaml 'BLOCKED — REPLAY/AUDIT DURABILITY NOT ACCEPTED ON CURRENT STACK' 'EAO registry records production replay/audit blocker'
...
require_text infrastructure/weknora/PROVISIONING.md 'Knowledge Mutation through EAO Admin is `BLOCKED / DISABLED`' 'WeKnora EAO Admin mutation surface is blocked'
...
require_no_text config/capabilities.yaml 'typed repository capability (currently unresolved; required before runtime enablement)' 'EAO blocked repository path does not block v1A enablement'
require_text config/capabilities.yaml 'status: review_only' 'EAO MCP control-plane review is explicitly review-only'
require_text config/capabilities.yaml 'runtime_mutation_binding_required: false' 'EAO review-only MCP path does not imply mutation binding'
require_text config/capabilities.yaml 'operation-envelope-signing-key' 'EAO registry declares reference-only approval signing secret class'
...
require_text infrastructure/open-webui/EAO-ADMIN-PROVISIONING.md 'disabled/reference-only operation-envelope HMAC binding' 'EAO provisioning marks approval binding reference-only'
...
require_text config/capabilities.yaml 'accepted_runtime_scope: review_and_recommendation_only' 'EAO Admin Action is reference-only outside accepted review scope'
require_text config/capabilities.yaml 'knowledge_read_only_verification' 'EAO Admin accepted scope includes read-only Knowledge verification'
require_text config/capabilities.yaml 'knowledge_mutation' 'EAO Admin forbids Knowledge Mutation'
require_no_text config/capabilities.yaml 'result: "PASS — EAO ADMIN CONSOLE V1A"' 'EAO readiness does not use the old mutation-inclusive PASS claim'
...
require_text infrastructure/open-webui/eao_admin_knowledge_action.py 'OUTCOME_UNKNOWN' 'Reference Action retains fail-closed replay semantics'
