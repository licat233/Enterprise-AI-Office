...
def social_probe(audit_script: Path | None) -> list[str]:
    ...
    with tempfile.TemporaryDirectory(prefix="phase5a-social-vault-") as vault_temp, tempfile.TemporaryDirectory(prefix="phase5a-social-input-") as input_temp:
        ...
        try:
            ...
            return failures
        check(names == {"route_work_product", "save_article_package", "save_social_package", "save_mic_product_package", "save_website_product_materials_package"}, "Social MCP exposes the closed Router, Article, Social, MIC, and Website Product Materials tools", failures)
...
def check_runtime(runtime_root: Path, hermes_home: Path, vault_root: Path) -> list[str]:
    ...
    check(set(router_include) == {"route_work_product", "save_article_package", "save_social_package", "save_mic_product_package", "save_website_product_materials_package"}, "Operations receives the exact scoped Router tool allowlist", failures)
